...
let packageName =
  ...
  const handleEdit = async (sellerId) => {
    try {
      ...
      // Merge nested seller/company/kyc/bank objects — same pattern as handleView
      let processedData = {};
      if (sellerData.seller) {
        processedData = {
          ...sellerData.seller,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!nameRegex.test(formData.name)) {
        throw new Error('Seller Name can only contain letters, spaces, and hyphens');
      }
      if (formData.name.length > 50) {
        throw new Error('Seller Name cannot exceed 50 characters');
      }
      ...
      const gstNumber = formData.company_GST_number?.trim();
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-4 md:hidden">
              {filteredSellers.map((seller, index) => (
                <div key={seller.id} className="bg-gray-50 rounded-xl border border-gray-100 p-3">
                  <div className="flex items-center gap-2 mb-2">
                    <div className="h-9 w-9 bg-gradient-to-br from-blue-500 to-blue-600 rounded-full flex items-center justify-center flex-shrink-0">
                      <span className="text-white font-bold text-xs">{seller.name?.charAt(0).toUpperCase() || 'U'}</span>
                    </div>
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-gray-900 truncate">{seller.name || 'N/A'}</p>
                      <p className="text-xs text-gray-500 truncate">{seller.email || 'N/A'}</p>
                      <p className="text-xs text-gray-400">{seller.mobile || 'N/A'}</p>
                    </div>
                  </div>
                  {seller.company_name && <p className="text-xs text-gray-600 mb-1 truncate">{seller.company_name}</p>}
                  {seller.package_name && seller.package_name !== 'No Package' && (
                    <p className="text-xs text-green-700 mb-1">{seller.package_name}</p>
                  )}
                  <div className="flex items-center justify-between mt-2">
                    ...
                    <div className="flex gap-1">
                      <button onClick={() => handleView(seller.id)} className="p-1.5 bg-blue-100 text-blue-700 rounded" title="View"><FiEye className="w-3 h-3" /></button>
                      <button onClick={() => handleEdit(seller.id)} className="p-1.5 bg-green-100 text-green-700 rounded" title="Edit"><FiEdit2 className="w-3 h-3" /></button>
                      <button onClick={() => handleDelete(seller)} className="p-1.5 bg-red-100 text-red-700 rounded" title="Delete"><FiTrash2 className="w-3 h-3" /></button>
                      <button onClick={() => handleViewProducts(seller)} className="p-1.5 bg-purple-100 text-purple-700 rounded" title="Products"><FiPackage className="w-3 h-3" /></button>
                    </div>
                  </div>
                </div>
              ))}
              ...
              {error && (
                <div className="mb-6 bg-red-50 border-l-4 border-red-500 p-4 rounded-r-lg shadow-sm">
                  <div className="flex items-start">
                    ...
                    <button type="button" onClick={() => setError('')} className="ml-3 text-red-500 hover:text-red-700">
              ...
              {productsError && (
                <div className="mb-6 bg-red-50 border-l-4 border-red-500 p-4 rounded-r-lg shadow-sm">
                  <div className="flex items-start">
                    ...
                    <button type="button" onClick={() => setProductsError('')} className="ml-3 text-red-500 hover:text-red-700">
                      <FiX className="text-lg" />
                    </button>
                  </div>
                </div>
