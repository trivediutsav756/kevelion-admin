...
const Category = () => {
  ...
  const handleAddCategory = async (e) => {
    ...
    if (!nameRegex.test(formData.category_name.trim())) {
      setError('Category name can only contain letters, spaces, and hyphens');
      setSubmitLoading(false);
      return;
    }
  ...
  return (
    <div className="p-4 sm:p-6">
      ...
      <div className="mb-5">
        ...
        <p className="text-gray-500 text-sm mt-0.5">Manage all your product categories</p>
      </div>

      {/* Count + Add Button */}
      <div className="flex flex-wrap justify-between items-center mb-5 gap-3">
        <h2 className="text-base sm:text-lg font-semibold text-gray-700">All Categories ({categories.length})</h2>
        <button
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        {loading && !showAddEditModal && !showViewModal ? (
          ...
          <div className="text-center py-12 text-gray-400">
            <FiImage className="mx-auto text-5xl mb-3" />
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 md:hidden">
              {categories.map((category, index) => (
                <div key={category.id} className="flex items-center gap-3 bg-gray-50 rounded-xl p-3 border border-gray-100">
                  {category.image ? (
                    <img src={`${API_BASE_URL}${category.image}`} alt={category.category_name} className="w-14 h-14 rounded-lg object-cover border flex-shrink-0"
                      onError={(e) => { e.target.src = 'https://via.placeholder.com/56?text=?'; }} />
                  ) : (
                    <div className="w-14 h-14 rounded-lg bg-gray-100 flex items-center justify-center border flex-shrink-0"><FiImage className="text-gray-400 text-xl" /></div>
                  )}
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-gray-400">#{index + 1}</p>
                    <p className="text-sm font-semibold text-gray-900 truncate">{category.category_name || 'N/A'}</p>
                  </div>
                  <div className="flex gap-1">
                    <button onClick={() => handleViewCategory(category.id)} className="p-2 text-blue-500 hover:bg-blue-50 rounded-lg"><FiEye /></button>
                    <button onClick={() => handleOpenEditModal(category.id)} className="p-2 text-green-500 hover:bg-green-50 rounded-lg"><FiEdit2 /></button>
                    <button onClick={() => handleDeleteCategory(category.id, category.category_name)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg"><FiTrash2 /></button>
                  </div>
                </div>
              ))}
            </div>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {categories.map((category, index) => (
                    <tr key={category.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-mono">{category.id || 'N/A'}</td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        {category.image ? (
                          <img src={`${API_BASE_URL}${category.image}`} alt={category.category_name} className="h-12 w-12 rounded-lg object-cover border"
                            onError={(e) => { e.target.src = 'https://via.placeholder.com/48?text=?'; }} />
                        ) : (
                          <div className="h-12 w-12 rounded-lg bg-gray-100 flex items-center justify-center border"><FiImage className="text-gray-400 text-xl" /></div>
                        )}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{category.category_name || 'N/A'}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex items-center gap-2">
                          <button onClick={() => handleViewCategory(category.id)} className="p-2 text-blue-500 hover:bg-blue-50 rounded-md"><FiEye /></button>
                          <button onClick={() => handleOpenEditModal(category.id)} className="p-2 text-green-500 hover:bg-green-50 rounded-md"><FiEdit2 /></button>
                          <button onClick={() => handleDeleteCategory(category.id, category.category_name)} className="p-2 text-red-500 hover:bg-red-50 rounded-md"><FiTrash2 /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
