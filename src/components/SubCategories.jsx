      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-8">
          ...
          <div className="mt-8 flex justify-end">
            <button
              onClick={onClose}
              className="bg-gray-600 text-white py-2 px-6 rounded-lg font-medium hover:bg-gray-700 transition duration-200"
            >
              Close
            </button>
          </div>
        ...
        <form onSubmit={onSubmit} className="p-6">
          ...
          <div className="flex gap-3 mt-6">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 bg-gray-300 text-gray-700 py-2 px-4 rounded-lg font-medium hover:bg-gray-400 transition duration-200"
              disabled={loading}
            >
              Cancel
            </button>
            <button
              type="submit"
              className="flex-1 bg-gradient-to-r from-blue-500 to-indigo-600 text-white py-2 px-4 rounded-lg font-medium hover:from-blue-600 hover:to-indigo-700 transition duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
              disabled={loading}
            >
              {loading ? (
...
const SubCategory = () => {
  ...
  const handleAddSubCategory = async (e) => {
    ...
    if (!nameRegex.test(currentSubCategory.subcategory_name.trim())) {
      setError('Subcategory name can only contain letters, spaces, and hyphens');
      return;
    }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-5">
          <div className="flex flex-wrap justify-between items-center mb-3 gap-3">
            <div>
              <h1 className="text-xl sm:text-3xl font-bold text-gray-800">SubCategory Management</h1>
              <p className="text-gray-500 text-sm mt-0.5">Manage all your product subcategories</p>
            </div>
            <button
              onClick={handleAddClick}
              className="bg-blue-600 text-white px-4 py-2 rounded-lg font-semibold hover:bg-blue-700 transition duration-200 shadow-md flex items-center gap-2 text-sm"
            >
          ...
          <div className="bg-white rounded-lg shadow-sm p-3">
            <span className="text-sm sm:text-base font-semibold text-gray-700">All SubCategories ({subcategories.length})</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          {(loading || apiLoading) && !showModal && !showViewModal ? (
            ...
            <div className="text-center py-12 text-gray-500">
              ...
              <p className="text-sm text-gray-400 mt-1">Add your first subcategory to get started</p>
            </div>
          ) : (
            <>
              {/* Mobile: Cards */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 md:hidden">
                {subcategories.map((subcategory, index) => {
                  const getCategoryName = (id) => { const c = categories.find(cat => cat.id === id); return c ? c.category_name : 'N/A'; };
                  return (
                    <div key={subcategory.id} className="flex items-center gap-3 bg-gray-50 rounded-xl p-3 border border-gray-100">
                      {subcategory.image ? (
                        <img src={`${API_BASE_URL}${subcategory.image}`} alt={subcategory.subcategory_name} className="w-14 h-14 rounded-lg object-cover border flex-shrink-0"
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs text-gray-400">#{index+1} · {getCategoryName(subcategory.category_id)}</p>
                        <p className="text-sm font-semibold text-gray-900 truncate">{subcategory.subcategory_name || 'N/A'}</p>
                      </div>
                      <div className="flex gap-1">
                        <button onClick={() => handleViewClick(subcategory.id)} className="p-2 text-blue-500 hover:bg-blue-50 rounded-lg"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg></button>
                        <button onClick={() => handleEditClick(subcategory.id)} className="p-2 text-green-500 hover:bg-green-50 rounded-lg"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
                        <button onClick={() => handleDeleteSubCategory(subcategory.id)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                      </div>
                    </div>
                  );
                })}
              ...
              <div className="hidden md:block overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-100">
                    <tr>
                      {['#','ID','Image','Category','Subcategory','Actions'].map(h => (
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {subcategories.map((subcategory, index) => (
                      <SubCategoryRow key={subcategory.id} subcategory={subcategory} index={index} categories={categories}
                        onView={handleViewClick} onEdit={handleEditClick} onDelete={handleDeleteSubCategory} />
                    ))}
                  </tbody>
                </table>
              </div>
